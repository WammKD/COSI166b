...
class MovieData
  ...
  # similarity by 0.1 and do it again.
  def predict(u, m)
    u, m = u.to_i, m.to_i
    # Finds all possible sim. users that've reviewed movie m unless none exist
    def shared_sim_users(us, mo, n = 5)
      return [] if(n < 1) # If we've run so many times that we're going below
                          # 1.0 (an impossible sim.), there're no similar users
      ...
      # If we wind up empty after that filtering, run again but w/ .1 less sim.
